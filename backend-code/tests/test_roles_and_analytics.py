...
def test_unauthenticated_access_blocked(api_client):
    """Verifies that protected processing queues and manager endpoints block unauthenticated requests with 401, while public application session creation succeeds."""
    endpoints = [
    ...
    for method, path in endpoints:
        r = api_client.get(path) if method == "GET" else api_client.post(path)
        assert r.status_code == 401, f"{method} {path} should return 401 without auth, got {r.status_code}"

    # Public applicant session creation should succeed (200 OK)
    pub_resp = api_client.post("/api/applications", data={"loan_type": "personal_loan"})
    assert pub_resp.status_code == 200, f"Public application creation should return 200, got {pub_resp.status_code}"
